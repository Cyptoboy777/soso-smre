  ...
  return (
    <div style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: 16, padding: 20, marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 11, color: currentValue >= initialBalance ? '#00e676' : '#f43f5e', fontWeight: 700 }}>
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
          <defs>
            <linearGradient id="grad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={currentValue >= initialBalance ? '#00e676' : '#f43f5e'} stopOpacity="0.2" />
              <stop offset="100%" stopColor={currentValue >= initialBalance ? '#00e676' : '#f43f5e'} stopOpacity="0" />
            </linearGradient>
          </defs>
          <path d={areaPath} fill="url(#grad)" />
          <path d={pathData} fill="none" stroke={currentValue >= initialBalance ? '#00e676' : '#f43f5e'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
