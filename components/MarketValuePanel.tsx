...
function HeatTile({ ticker, onSelect, isSelected }: { ticker: Ticker; onSelect: (t: Ticker) => void; isSelected: boolean }) {
  ...
  const bg = pct > 8  ? 'rgba(43,217,168,0.55)'
           : pct > 4  ? 'rgba(43,217,168,0.35)'
           : pct > 1  ? 'rgba(43,217,168,0.18)'
           : pct > 0  ? 'rgba(43,217,168,0.09)'
           : pct > -1 ? 'rgba(255,107,107,0.09)'
           : pct > -4 ? 'rgba(255,107,107,0.18)'
           : pct > -8 ? 'rgba(255,107,107,0.35)'
           :             'rgba(255,107,107,0.55)';

  const textColor = pct >= 0 ? '#2bd9a8' : '#ff6b6b';
...
function AISignalRow({ ticker }: { ticker: Ticker }) {
  ...
  const color  = signal === 'STRONG BUY' ? '#2bd9a8' : signal === 'BUY' ? '#69f0ae' : signal === 'NEUTRAL' ? '#ffd740' : signal === 'CAUTION' ? '#f97316' : '#ff6b6b';
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', background:'rgba(255,255,255,0.025)', borderRadius:10, border:'1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        <span style={{ fontSize:12, fontWeight:900, color: pct >= 0 ? '#2bd9a8' : '#ff6b6b', fontFamily:'var(--font-mono)' }}>
...
export default function MarketValuePanel({ tickers, selected, onSelect }: Props) {
  ...
  const sentimentColor = sentiment === 'BULLISH' ? '#2bd9a8' : sentiment === 'BEARISH' ? '#ff6b6b' : '#ffd740';
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
        ...
        <StatCard label="Gainers" value={`${gainers.length}`} sub={`of ${tickers.length} pairs`} color="#2bd9a8"/>
        <StatCard label="Losers"  value={`${losers.length}`}  sub={`of ${tickers.length} pairs`} color="#ff6b6b"/>
        <StatCard label="Top Gainer" value={`+${top5[0]?.priceChangePct.toFixed(1)}%`} sub={top5[0]?.base} color="#2bd9a8"/>
      </div>
      ...
      {view === 'stats' && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
          {/* Top Gainers */}
          <div style={{ background:'rgba(43,217,168,0.04)', border:'1px solid rgba(43,217,168,0.15)', borderRadius:14, padding:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
              <TrendingUp size={12} color="#2bd9a8"/>
              <span style={{ fontSize:9, fontWeight:900, color:'#2bd9a8', letterSpacing:'.12em' }}>TOP GAINERS</span>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {top5.map((t,i) => (
                <div key={t.symbol} onClick={() => onSelect(t)} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', padding:'4px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <span style={{ fontSize:12, fontWeight:900, color:'#2bd9a8', fontFamily:'var(--font-mono)' }}>+{t.priceChangePct.toFixed(2)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Top Losers */}
          <div style={{ background:'rgba(255,107,107,0.04)', border:'1px solid rgba(255,107,107,0.15)', borderRadius:14, padding:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
              <TrendingDown size={12} color="#ff6b6b"/>
              <span style={{ fontSize:9, fontWeight:900, color:'#ff6b6b', letterSpacing:'.12em' }}>TOP LOSERS</span>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {bottom5.map((t,i) => (
                <div key={t.symbol} onClick={() => onSelect(t)} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', padding:'4px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                  <span style={{ fontSize:12, fontWeight:900, color:'#fff' }}>{i+1}. {t.base}</span>
                  <span style={{ fontSize:12, fontWeight:900, color:'#ff6b6b', fontFamily:'var(--font-mono)' }}>{t.priceChangePct.toFixed(2)}%</span>
                </div>
              ))}
            </div>
          </div>
